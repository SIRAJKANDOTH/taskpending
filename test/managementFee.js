...
const PerformanceManagementFee = artifacts.require(
...
contract(" APContract", function (accounts) {
  ...
  beforeEach(async function () {
	...
	performanceManagementFee=await PerformanceManagementFee.new()
  ...
  it("Execute management fee clean Up", async () => {
    ...
    console.log(
      "sender balance",
      (await newYieldsterVault.balanceOf(accounts[0])).toString()
    );
    ...
    assert.equal(
      parseInt(result),
      parseInt(calculatedFee),
      "expected and calculated management Fee matches"
    );
  });

  it("Performance based management fee", async () => {
	await yrtToken.approve(newYieldsterVault.address, token("100"));
    await newYieldsterVault.deposit(yrtToken.address, token("100"));
    await aishToken.approve(newYieldsterVault.address, token("100"));
    await newYieldsterVault.deposit(aishToken.address, token("100"));
