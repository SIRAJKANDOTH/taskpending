...
var LivaOne = artifacts.require("./strategies/LivaOne.sol");
...
contract(" Deposit", function (accounts) {
  ...
  beforeEach(async function () {
    ...
    managementFee = await PlatformManagementFee.deployed();
    hexUtils = await HexUtils.deployed();
    ...
    livaOne = await LivaOne.new(apContract.address, [
      "0xd27Dc2D8ceF541f94FbA737079F2DFeA39B2EEf8",
      "0xD8052918CAd9a8B3a564d7Aa4e680a0dc156380e",
      "0x3662ABD754eE1d8CB6f5F1D4E315932b36e9955B",
    ]);
    ...
    // Add strategy minter
    yearnitallMinter = await YearnItAllMinter.new(
      apContract.address,
      yearnItAll.address
    );
    livaOneMinter = await LivaOneMinter.new(
      apContract.address,
      livaOne.address
    );
  ...
  it("should create a vault with Liva One strategy", async () => {
    let newYieldsterVaultData = await yieldsterVaultMasterCopy.contract.methods
      .setup(
        "Liva One",
        "Liva",
        "LV",
    ...
    yieldsterVaultLiva = await utils.getParamFromTxEvent(
      await proxyFactory.createProxy(newYieldsterVaultData),
      "ProxyCreation",
      "proxy",
      proxyFactory.address,
    ...
    console.log(
      "vault owner",
      await yieldsterVaultLiva.owner(),
      "other address",
      accounts[0]
    );

    await yieldsterVaultLiva.registerVaultWithAPS();

    console.log("vault Name", await yieldsterVaultLiva.vaultName());
    assert.equal(
      await yieldsterVaultLiva.vaultName(),
      "Liva One",
      "Names match"
    );

    let vaultNAVInitial = await yieldsterVaultLiva.getVaultNAV();
    console.log("vault NAV Initial", vaultNAVInitial.toString());

    let vaultTokenValueInitial = await yieldsterVaultLiva.tokenValueInUSD();
    console.log("token value usd Initial", vaultTokenValueInitial.toString());

    await yieldsterVaultLiva.setVaultStrategyAndProtocol(
      livaOne.address,
      [
        "0xd27Dc2D8ceF541f94FbA737079F2DFeA39B2EEf8",
        "0xD8052918CAd9a8B3a564d7Aa4e680a0dc156380e",
    ...
    await yieldsterVaultLiva.setVaultActiveStrategy(livaOne.address);
