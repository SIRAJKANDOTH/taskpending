...
contract ManagementFee is VaultStorage {
    function processPlatformFee(uint256 blockDifference, uint256 vaultNAV)
        internal
    {
        ManagementFeeStorage mStorage =
            ManagementFeeStorage(0xe40462adEf00dd036eF5B1C57862242B9205b658);
        uint256 platformFee = mStorage.getPlatformFee();
        uint256 platformNavInterest =
            vaultNAV.mul(blockDifference.mul(1e18)).mul(platformFee).div(
                uint256(262800000).mul(1e36)
            );
        uint256 platformShare =
            platformNavInterest.mul(1e18).div(tokenValueInUSD());
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < strategies.length; i++) {
            (address benefeciary, ) =
                IAPContract(APContract).getStrategyManagementDetails(
                    address(this),
                    strategies[i]
                );
            uint256 strategyFee = mStorage.getStrategyFee(strategies[i]);
            if (strategyFee > 0) {
                uint256 strategyAnnualNavInterest =
                    vaultNAV
                        .mul(blockDifference.mul(1e18))
                        .mul(strategyFee)
                        .div(uint256(262800000).mul(1e36));
                uint256 strategyShare =
                    strategyAnnualNavInterest.mul(1e18).div(tokenValueInUSD());
