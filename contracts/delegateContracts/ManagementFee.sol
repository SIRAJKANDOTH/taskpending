    ...
    VaultStorage 
{
        ...
        internal 
    {
        ...
        for (uint256 i = 0; i < strategies.length; i++) {
            ...
            if(strategyFee > 0) {
                uint256 strategyAnnualNavInterest = vaultNAV.mul(blockDifference.mul(1e18)).mul(strategyFee).div(uint256(262800000).mul(1e36));
                uint256 strategyShare = strategyAnnualNavInterest.mul(1e18).div(tokenValueInUSD());
